...
namespace NUXML
{
    ...
    public class FieldBinding
    {
        ...
        public void Initialize()
        {
            ...
            {
                Debug.LogError(String.Format("[.375] Invalid field binding: [{0}].{1} <-> [{2}].{3}. Source and target view can't be the same.", actualSource.name, SourceFieldPathString, actualTarget.name, TargetFieldPathString));
        ...
        public static KeyValuePair<View, string>? ParseFieldPath(string fieldPath, View sourceView, out object targetObject, out FieldInfo objectFieldInfo, out FieldInfo viewFieldInfo)
        {
            ...
            for (int i = 0; i < fields.Length; ++i)
            {
                ...
                if (end > 0)
                {
                    ...
                    {
                        Debug.LogError(String.Format("[.301] {0}: Unable to parse field path \"{1}\".", sourceView.Name, fieldPath));
                        return null;
                    }
                ...
                if (fieldInfo == null)
                {
                    ...
                    objectFieldInfo = null;
                    Debug.LogError(String.Format("[.302] {0}: Unable to parse field path \"{1}\". Couldn't find field/view \"{2}\".", sourceView.Name, fieldPath, fields[i]));
                    return null;
                }
                ...
                if (fieldInfo.FieldType.IsSubclassOf(typeof(View)) || fieldInfo.FieldType == typeof(View))
                {
                    ...
                    if (i == fields.Length - 1)
                    {
                        ...
                        Debug.LogError(String.Format("[.303] {0}: Unable to parse field path \"{1}\". Last member access can't be a view.", sourceView.Name, fieldPath));
                        return null;
                    }
                ...
                if (i == fields.Length - 1)
                {
                    ...
                    if (index != -1)
                    {
                        Debug.LogError(String.Format("[.304] {0}: Unable to parse field path \"{1}\". Last member access can't be indexed.", sourceView.Name, fieldPath));
                        return null;
                    }
                ...
                else
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        {
                            Debug.LogError(String.Format("[.305] {0}: Unable to parse field path \"{1}\". Unable to retrieve indexed object \"{2}\".", sourceView.Name, fieldPath, fields[i]));
                            return null;
                        }
