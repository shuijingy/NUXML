...
namespace NUXML
{
    ...
    public class ViewFieldAnimator
    {
        ...
        {           
            ...
            if (!_isInitialized)
            {
                Debug.LogWarning("[.372] Animator started before setting target view. Make sure you call SetAnimationTarget() before starting animation.");
                return;
            }

            if (_targetFieldInfo == null)
            {
                Debug.LogWarning("[.348] Animator started before setting animated field.");
                return;
            }
        ...
        internal void UpdateAnimation(float deltaTime)
        {
            ...
            catch (Exception e)
            {
                ...
                Debug.LogError(String.Format("[.316] Animate: Unable to animate field {0}.{1}. Interpolator {2} threw exception: {3}. Stopping animation.", viewName, fieldName, _valueInterpolator.GetType().Name, e.Message));
                _active = false;
                return;
            }
        ...
        public void SetAnimationTarget(View target)
        {
            ...
            for (int i = 0; i < fields.Length; ++i)
            {
                ...
                if (end > 0)
                {
                    ...
                    {
                        Debug.LogError(String.Format("[.317] Unable to parse animation target field path \"{1}\".", Field));
                        return;
                    }
                ...
                if (fieldInfo == null)
                {
                    ...
                    _targetFieldInfo = null;
                    Debug.LogError(String.Format("[.318] Unable to parse animation target field path \"{1}\". Couldn't find field/view \"{2}\".", Field, fields[i]));
                    return;
                }
                ...
                if (i == fields.Length - 1)
                {
                    ...
                    if (index != -1)
                    {
                        Debug.LogError(String.Format("[.319] Unable to parse animation target field path \"{1}\". Last member access can't be indexed.", Field));
                        return;
                    }
                ...
                {
                    Debug.LogError(String.Format("[.320] Unable to parse animation target field path \"{1}\". Object \"{2}\" in path was null.", Field, fields[i]));
                    return;
                }
            }
            ...
            if (From == null && !String.IsNullOrEmpty(FromStringValue))
            {
                ...
                if (!result.Success)
                {
                    Debug.LogError(String.Format("[.321] Unable to parse animation FromValue \"{1}\". {2}", From, result.ErrorMessage));
                    return;
                }
            ...
            if (To == null && !String.IsNullOrEmpty(ToStringValue))
            {
                ...
                if (!result.Success)
                {
                    Debug.LogError(String.Format("[.322] Unable to parse animation ToValue \"{1}\". {2}", To, result.ErrorMessage));
                    return;
                }
