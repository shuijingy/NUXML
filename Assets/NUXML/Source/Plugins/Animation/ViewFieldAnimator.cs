...
namespace NUXML.Animation
{
    ...
    public class ViewFieldAnimator
    {
        ...
        {           
            ...
            if (TargetView == null)
            {
                Debug.LogWarning("[NUXML] Animator started before setting target view.");
                return;
            }

            if (String.IsNullOrEmpty(Field))
            {
                Debug.LogWarning("[NUXML] Animator started before setting animation view field.");
                return;
            }
            ...
            {
                Debug.LogError(String.Format("[NUXML] Unable to animate view field \"{0}\" on view \"{1}\"\". Field not found.", Field, TargetView.GameObjectName));
                return;
            }
            ...
            if (From == null && !String.IsNullOrEmpty(FromStringValue))
            {
                ...
                if (!result.Success)
                {
                    Debug.LogError(String.Format("[NUXML] Unable to parse animation From value \"{0}\". {1}", From, result.ErrorMessage));
                    return;
                }
            ...
            if (To == null && !String.IsNullOrEmpty(ToStringValue))
            {
                ...
                if (!result.Success)
                {
                    Debug.LogError(String.Format("[NUXML] Unable to parse animation To value \"{0}\". {1}", To, result.ErrorMessage));
                    return;
                }
        ...
        {            
            ...
            {
                Debug.LogError(String.Format("[NUXML] Unable to animate field \"{0}\" on view \"{1}\". Stopping animation. Interpolator {2} threw exception: {3}", Field, TargetView.GameObjectName, _valueInterpolator.GetType().Name, Utils.GetError(e)));
                _isRunning = false;
                return;
            }
            ...
            {
                Debug.LogError(String.Format("[NUXML] Unable to animate field \"{0}\" on view \"{1}\". Stopping animation. Exception thrown when trying to set view field value to \"{2}\": {3}", Field, TargetView.GameObjectName, interpolatedValue, Utils.GetError(e)));
                _isRunning = false;
                return;
            }
