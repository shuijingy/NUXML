...
namespace NUXML
{
	...
	/// </summary>
	public class NGUIBindingValueObserver : NGUIValueObserver
	{
		#region Fields
		...
		/// </summary>
		public NGUIBindingValueObserver()
		{
		...
		public override bool Notify(HashSet<NGUIViewFieldData> callstack)
		{
			try
			{
				...
				if (TargetNGUI.SourceNGUIView == null)
				{
					return false; 
				}
		...
		private void PrintBindingError(Exception e)
		{
			...
			default:
			case BindingType.SingleBinding:
				Debug.LogError(String.Format("[NUXML] Exception thrown when propagating single binding value from source \"{0}\" to target \"{1}.{2}\": {3}", Sources[0].BindingSourceString, TargetNGUI.SourceNGUIView.ViewTypeName, TargetNGUI.ViewFieldPath, Utils.GetError(e)));
				 break;
			...
			case BindingType.MultiBindingFormatString:
				...
				Debug.LogError(String.Format("[NUXML] Exception thrown when propagating single binding value from sources \"{0}\" to target \"{1}.{2}\": {3}", sb.ToString(), TargetNGUI.SourceNGUIView.ViewTypeName, TargetNGUI.ViewFieldPath, Utils.GetError(e)));
				 break;
			}
		}    
