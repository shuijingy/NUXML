...
namespace NUXML
{
	...
	/// </summary>
	[Serializable]
	public class NGUIViewActionEntry
	{
		#region Fields
		...
		internal void Invoke(ActionData actionData, BaseEventData baseEventData, object customData)
		{
			...
			try
			{
				_viewActionMethod.Invoke(ParentNGUIView, _parameters);
			}
			catch (Exception e)
			{
				Debug.LogError(String.Format("[NUXML] {0}: Exception thrown when triggering view action handler \"{1}.{2}()\" for view action \"{3}\": {4}", SourceNGUIView.GameObjectName, ParentNGUIView.ViewTypeName, ViewActionHandlerName, ViewActionFieldName, Utils.GetError(e)));
			}
		}
		...
		private void Initialize()
		{
			...
			_viewActionMethod = ParentNGUIView.GetType().GetMethod(ViewActionHandlerName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (_viewActionMethod == null)
			{
				Debug.LogError(String.Format("[NUXML] {0}: Unable to initialize view action handler \"{1}.{2}()\" for view action \"{3}\". View action handler not found.", SourceNGUIView.GameObjectName, ParentNGUIView.ViewTypeName, ViewActionHandlerName, ViewActionFieldName));
				return;
			}
			...
			for (int i = 0; i < parameterCount; ++i)
			{
				...
				{
					if (!viewActionMethodParameters[i].ParameterType.IsAssignableFrom(SourceNGUIView.GetType()))
					{
						Debug.LogError(String.Format("[NUXML] View action \"{0}.{1}\" " +
													 "has parameter \"{2}\" with invalid type. " +
													 "Expected type (or baseclass of) \"{3}\".", 
													  ParentNGUIView.ViewTypeName, 
													  ViewActionHandlerName, 
													  viewActionMethodParameters[i].Name, 
													  SourceNGUIView.ViewTypeName));
					}

					_parameters[i] = SourceNGUIView;
				}
