...
namespace NUXML
{
	...
	public static class NGUIViewData
	{
		...
		public static void GenerateNGUIViews()
		{
			...
			viewPresenter.NGUIViews.Clear();
			viewPresenter.NGUIViews.AddRange(viewPresenter.ViewTypeDataList.Where(y => !y.HideInPresenter).Select(x => x.ViewName).OrderBy(x => x));
            ...
            foreach (var viewType in viewPresenter.ViewTypeDataList)
            {
                ...
                foreach (var dependencyName in viewType.DependencyNames)
                {
                    ...
                    if (dependency == null)
                    {
                        Utils.LogError("[NUXML] {0}: View contains the child view \"{1}\" that could not be found.", viewType.ViewName, dependencyName);
                        continue;
                    }
            ...
            {
                Utils.LogError("[NUXML] Unable to generate views. {0}", e.Message);
                return;
            }
            ...
            if (viewPresenter.transform.childCount > 0)
            {
                for (int i = viewPresenter.transform.childCount - 1; i >= 0; --i)
                {
                    ...
                    Utils.LogWarning("[NUXML] Removed GameObject \"{0}\" under the view-presenter. View-presenter content is reserved for objects generated by the framework.", go.name);
			...
			// initialize views
			viewPresenter.InitializeNGUIViews();
		}
        ...
        public static void LoadXuml(string xuml, string xumlAssetName = "")
        {
            ...
            {
                Utils.LogError("[NUXML] {0}: Error parsing XUML. Exception thrown: {1}", xumlAssetName, Utils.GetError(e));
                return;
            }
        ...
        private static void LoadThemeXuml(XElement xumlElement, string xuml, string xumlAssetName)
        {
            ...
            {
                Utils.LogError("[NUXML] {0}: Error parsing theme XUML. Name attribute missing.", xumlAssetName);
            }
            ...
            {           
                ...
                else
                {
                    ...
                    {
                        Utils.LogError("[NUXML] {0}: Error parsing theme XUML. Unable to parse UnitSize attribute value \"{1}\".", xumlAssetName, unitSizeAttr.Value);
                        themeData.UnitSize = ViewPresenter.Instance.UnitSize;
                    }
                }
            }
        ...
        private static List<Resource> LoadResourceXuml(string xumlAssetName, XElement xumlElement, string parentKey, string parentValue, string parentLanguage, string parentPlatform)
        {
            ...
            foreach (var childElement in xumlElement.Elements())
            {
                ...
                {
                    Utils.LogError("[NUXML] {0}: Error parsing resource dictionary XUML. Unrecognizable element \"{1}\".", xumlAssetName, childElement.Name.LocalName);
                    continue;
                }
...
//			}
			...
			foreach (var referenceField in viewTypeData.ReferenceFields)
			{
				...
				var referencedView = view.FindNGUI<NGUIView>(x => String.Equals(x.Id, referenceField, StringComparison.OrdinalIgnoreCase),
					true, view);
        ...
        private static ValueConverterContext GetValueConverterContext(ValueConverterContext parentContext, XElement element, string viewName)
        {
            ...
            if (unitSizeAttr != null)
            {
                ...
                {
                    Utils.LogError("[NUXML] {0}: Error parsing XUML. Unable to parse UnitSize attribute value \"{1}\".", viewName, unitSizeString);
		...
		private static void SetViewValues(NGUIView view, XElement xumlElement, NGUIView parent, ValueConverterContext context)
		{
            ...
            foreach (var attribute in xumlElement.Attributes())
            {
                ...
                if (notAllowed)
                {
                    Utils.LogError("[NUXML] {0}: Unable to assign value \"{1}\" to view field \"{2}.{3}\". Field not allowed to be set from XUML.", view.GameObjectName, viewFieldValue, view.ViewTypeName, viewFieldPath);
                    continue;
                }
                ...
                {
                    Utils.LogError("[NUXML] {0}: Unable to assign value \"{1}\" to view field \"{2}\". View field not found.", view.GameObjectName, viewFieldValue, viewFieldPath);
                    continue;
                }
        ...
        {
            return NGUIViewPresenter.Instance.GetViewTypeData(viewTypeName);
        }
        ...
        {
			return NGUIViewPresenter.Instance.GetThemeData(themeName);
        }
        ...
        {
			return NGUIViewPresenter.Instance.GetViewType(viewTypeName);
        }
        ...
        {
			return NGUIViewPresenter.Instance.GetValueConverterForType(viewFieldType);
        }
        ...
        {
			return NGUIViewPresenter.Instance.GetValueConverter(valueConverterTypeName);
        }
        ...
        {
			return NGUIViewPresenter.Instance.GetValueInterpolatorForType(viewFieldType);
        }
