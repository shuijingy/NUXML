...
namespace NUXML
{
	...
	public class NGUIViewPresenter : NGUIView
	{
		...
		private static NGUIViewPresenter _instance;
		...
		/// </summary>
		public void InitializeNGUIViews(GameObject rootGameObject)
		{
			...
			InitializeNGUIViews(rootGameObject.GetComponent<NGUIView>());
		}
			
		public void InitializeNGUIViews(NGUIView rootNGUIView)
		{
			if (rootNGUIView == null || rootNGUIView.IsInitialized)
			{
				return;
			}

			rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryInitializeInternalDefaultValues());
			rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryInitializeInternal());
			rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryInitialize(),             true, null, TraversalAlgorithm.ReverseBreadthFirst);
			rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryPropagateBindings(),      true, null, TraversalAlgorithm.BreadthFirst);
			rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryQueueAllChangeHandlers(), true, null, TraversalAlgorithm.ReverseBreadthFirst);
			...
			int pass = 0;
			while (rootNGUIView.FindNGUI<NGUIView>(x => x.HasQueuedChangeHandlers) != null )
			{
				if (pass >= 1000)
				{
					PrintTriggeredChangeHandlerOverflowError(pass, rootNGUIView);
					break;
				}

				// as long as there are change handlers queued, go through all views and trigger them
				rootNGUIView.ForThisAndEachChildNGUI<NGUIView>(x => x.TryTriggerChangeHandlers(), true, null, TraversalAlgorithm.ReverseBreadthFirst);
				++pass;
			}            
		...
		/// </summary>
		private void PrintTriggeredChangeHandlerOverflowError(int pass, NGUIView rootView)
		{
			var sb = new StringBuilder();
			var triggeredViews = rootView.GetChildrenNGUI<NGUIView>(x => x.HasQueuedChangeHandlers);
		...
		public static void UpdateInstance()
		{
			...
			{
				_instance     = UnityEngine.Object.FindObjectOfType(typeof(NGUIViewPresenter)) as NGUIViewPresenter;
		...
		/// </summary>
		public static NGUIViewPresenter Instance
		{
			get
			{
				UpdateInstance();
