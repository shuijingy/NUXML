...
namespace NUXML
{
    ...
    public class ViewActionEntry
    {
        ...
        internal void Invoke(ActionData actionData, BaseEventData baseEventData, object customData)
        {
            ...
            {
                Utils.LogError("[NUXML] {0}: Exception thrown when triggering view action handler \"{1}.{2}()\" for view action \"{3}\": {4}", SourceView.GameObjectName, ParentView.ViewTypeName, ViewActionHandlerName, ViewActionFieldName, Utils.GetError(e));
            }
        }
        ...
        private void Initialize()
        {
            ...
            {
                Utils.LogError("[NUXML] {0}: Unable to initialize view action handler \"{1}.{2}()\" for view action \"{3}\". View action handler not found.", SourceView.GameObjectName, ParentView.ViewTypeName, ViewActionHandlerName, ViewActionFieldName);
                return;
            }
            ...
            for (int i = 0; i < parameterCount; ++i)
            {
                if (viewType.IsAssignableFrom(viewActionMethodParameters[i].ParameterType))
                {
                    ...
                    {
                        Utils.LogError("[NUXML] View action \"{0}.{1}\" has parameter \"{2}\" with invalid type. Expected type (or baseclass of) \"{3}\".", ParentView.ViewTypeName, ViewActionHandlerName, viewActionMethodParameters[i].Name, SourceView.ViewTypeName);
                    }
