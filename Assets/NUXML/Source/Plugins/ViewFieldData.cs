    ...
    public class ViewFieldData
    {
            ...
            ValueConverterContext context = null, bool notifyObservers = true)
        {
            ...
            if (!IsOwner)
            {
                ...
                if (targetView == null)
                {
                    Debug.LogError(String.Format("[NUXML] {0}: Unable to assign value \"{1}\" to view field \"{2}\". View along path is null.", SourceView.GameObjectName, inValue, ViewFieldPath));
                    return null;
                }
            ...
            if (!IsPathParsed)
            {
                ...
                if (!ParseViewFieldPath())
                {
                    ...
                    if (SevereParseError)
                    {
                        ...
                        Debug.LogError(String.Format("[NUXML] {0}: Unable to assign value \"{1}\". {2}", SourceView.GameObjectName, inValue, ParseError));
                    }
        ...
        public object GetValue(out bool hasValue)
        {
            if (!IsOwner)
            {
                ...
                {
                    hasValue = false;
                    //Debug.LogError(String.Format("[NUXML] {0}: Unable to get value from view field \"{1}\". View along path is null.", SourceView.GameObjectName, ViewFieldPath));
                    return null;
                }
